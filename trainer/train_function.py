...
class Trainer:
    ...
    def training_step(self, dataloader):
        ...
        for i, data in enumerate(tk0):
            ...
            # LABELS
            label_real = torch.ones(config.DCGAN.BATCH_SIZE, dtype=torch.float32, device=self.device)
            label_fake = torch.zeros(config.DCGAN.BATCH_SIZE, dtype=torch.float32, device=self.device)
            self.D.zero_grad()
            ...
            # DIS LOSS
            errD = self.loss.dis_loss(output_fake=output_fake, output_real=output_real)
            
            # WE ADD GRADIENT PENALTY IF WGAN LOSS
            if WGAN:
                errD_GP = self.loss.compute_gradient_penalty_loss(real_images=real_data, fake_images=fake, gp_scale=config.WGAN.LAMBDA_GP)
                errD = errD + errD_GP
            ...
            if i % config.WGAN.CRITICS_ITER == 0:
                label_gen = torch.ones(torch.ones(config.DCGAN.BATCH_SIZE, dtype=torch.float32, device=self.device)
                self.G.zero_grad()
                output_gen = self.D(fake).view(-1)
                errG = self.loss.gen_loss(output_fake=output_gen)
                errG.backward()
